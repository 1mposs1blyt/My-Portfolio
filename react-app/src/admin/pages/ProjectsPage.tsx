...
import {
  ...
  DELETE_PROJECT_IMAGE_MUTATION,
  DELETE_PROJECT_MUTATION,
  REORDER_PROJECT_IMAGES_MUTATION,
...
export default function ProjectsPage() {
  ...
  const [{ data, fetching, error }] = useQuery({ query: ADMIN_PROJECTS_QUERY });
  ...
  const save = async (item: Project) => {
    ...
    if (isNew) patch(item.id, { id: res.data.createProject.id });
    say("Сохранено");
  };
  ...
  const remove = async (item: Project) => {
    ...
    const tail = count
      ? ` и ${count} изображени${count === 1 ? "е" : "й"}`
      : "";
    const ok = await confirm({
      title: `Удалить проект «${item.name}»?`,
      text: count
        ? `Вместе с ним удалится изображений: ${count}. Это необратимо.`
        : undefined,
      danger: true,
    });
    if (!ok) return;
    ...
    const res = await deleteProject({ id: item.id });
    if (res.error) {
      say(res.error.message.replace("[GraphQL] ", ""));
  ...
  /** Загружает файл на /upload/image и привязывает результат к проекту */
  const upload = async (item: Project, file: File) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body.message ?? `Загрузка не удалась (${response.status})`,
        );
      }

      const { url } = await response.json();

      const res = await addImage({ input: { projectId: item.id, url } });
      if (res.error)
        throw new Error(res.error.message.replace("[GraphQL] ", ""));

      patch(item.id, { images: [...item.images, res.data.addProjectImage] });
  ...
  const dropImage = async (item: Project, image: ProjectImage) => {
    if (!(await confirm({ title: "Удалить изображение?", danger: true })))
      return;
    patch(item.id, { images: item.images.filter((i) => i.id !== image.id) });
    ...
    if (res.error) {
      say(res.error.message.replace("[GraphQL] ", ""));
      patch(item.id, { images: item.images });
    }
  };
  ...
  const moveImage = async (item: Project, index: number, dir: -1 | 1) => {
    ...
    patch(item.id, { images: next });

    if (!isEditor) return;

    const res = await reorderImages({ ids: next.map((i) => i.id) });
          ...
          <div className="adm-grid2">
            <div className="adm-field">
              ...
              <input
                ...
                onChange={(e) => patch(item.id, { repoUrl: e.target.value })}
              />
            </div>
            <div className="adm-field">
              <label>Демо</label>
              <input
                value={item.liveUrl ?? ""}
                placeholder="https://…"
                onChange={(e) => patch(item.id, { liveUrl: e.target.value })}
              />
            </div>
          </div>
          ...
          <div className="adm-field">
            ...
            <div className="adm-shots">
              {item.images.map((image, i) => (
                <figure className="adm-shot" key={image.id}>
                  <img src={src(image.url)} alt="" />
                  <figcaption>
                    <button
                      className="adm-icon"
                      onClick={() => moveImage(item, i, -1)}
                      disabled={i === 0}
                      aria-label="Левее"
                    >
                      ←
