...
/** GraphQL отдаёт даты как ISO-строку, для input[type=date] нужен YYYY-MM-DD */
...
export default function ExperiencePage() {
      ...
      list.map((e) =>
        e.id === id
          ? {
              ...
              achievements: [
                ...e.achievements,
                { text: "", order: e.achievements.length },
              ],
            }
          : e,
      ),
  ...
  const save = async (item: Experience) => {
    ...
    // достижения уходят массивом строк, порядок = позиция в массиве
    ...
    const res = isNew
      ? await createExperience({ input: payload })
      : await updateExperience({ input: { id: item.id, ...payload } });
    setSavingId(null);
  ...
  const remove = async (item: Experience) => {
    ...
    // новая запись ещё не в базе — удалять нечего
    ...
    const res = await deleteExperience({ id: item.id });
    if (res.error) {
      say(res.error.message.replace("[GraphQL] ", ""));
        ...
        <section className="adm-card" key={item.id}>
          <div className="adm-grid2">
            ...
            <div className="adm-field">
              ...
              <input
                value={item.position}
                onChange={(e) => patch(item.id, { position: e.target.value })}
              />
            </div>
          </div>
          ...
          <div className="adm-grid2">
            ...
            <div className="adm-field">
              ...
              <input
                ...
                value={item.endDate ?? ""}
                onChange={(e) =>
                  patch(item.id, { endDate: e.target.value || null })
                }
              />
