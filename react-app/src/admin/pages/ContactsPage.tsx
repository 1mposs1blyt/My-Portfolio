...
import { ADMIN_LINKS_QUERY, CREATE_LINK_MUTATION, DELETE_LINK_MUTATION, UPDATE_LINK_MUTATION } from "../api/api";
...
export default function ContactsPage() {
  ...
  const patch = (id: string, changes: Partial<Link>) => setRows(rs => rs.map(r => r.id === id ? {
    ...r,
    ...changes
  } : r));
  ...
  const commit = async (row: Link) => {
    ...
    const changed = !before || before.kind !== row.kind || before.label !== row.label || before.url !== row.url || before.order !== row.order;
    if (!changed) return;
        ...
        <tbody>
          {rows.map(row => <tr key={row.id}>
              ...
              commit(next);
                  ...
                  {KINDS.map(k => <option key={k.value} value={k.value}>
                      {k.label}
                    </option>)}
                </select>
              </td>
              <td>
                <input value={row.label} onChange={e => patch(row.id, {
              ...
              <td>
                <input value={row.url} onChange={e => patch(row.id, {
              url: e.target.value
            })} onBlur={() => commit({
              ...row
            })} />
              ...
              <td>
                <input className="adm-num" type="number" value={row.order} onChange={e => patch(row.id, {
              ...
              <td className="adm-td-right">
                <button className="adm-del" onClick={() => remove(row)} aria-label="Удалить">
                  ✕
                </button>
              </td>
            </tr>)}
          ...
          <tr className="adm-row-new">
            <td>
              <select value={draft.kind} onChange={e => setDraft(d => ({
              ...d,
              kind: e.target.value
            }))}>
                {KINDS.map(k => <option key={k.value} value={k.value}>
            ...
            <td>
              <input value={draft.label} placeholder="@username" onChange={e => setDraft(d => ({
              ...d,
              label: e.target.value
            }))} />
            </td>
            <td>
              <input value={draft.url} placeholder="https://t.me/username" onChange={e => setDraft(d => ({
              ...d,
              url: e.target.value
            ...
            <td />
            <td className="adm-td-right">
              <button className="adm-btn" onClick={add} disabled={!draft.label.trim() || !draft.url.trim()}>
                добавить
              </button>
            </td>
          </tr>
