...
import { ADMIN_PROJECTS_LIGHT_QUERY, ADMIN_REVIEWS_QUERY, ADMIN_REVIEW_TOKENS_QUERY, CREATE_REVIEW_TOKEN_MUTATION, DELETE_REVIEW_MUTATION, REVOKE_REVIEW_TOKEN_MUTATION } from "../api/api";
...
export default function ReviewsPage() {
  ...
  if (reviewsError) return <div className="adm-page adm-error">{reviewsError.message}</div>;
  return <div className="adm-page adm-page-wide">
      ...
      <section className="adm-card">
        ...
        <div className="adm-grid2">
          ...
          <div className="adm-field">
            ...
            <input type="number" min={1} max={365} value={form.days} onChange={e => setForm(f => ({
            ...f,
        ...
        {form.type === "CLIENT" && <div className="adm-field">
            <label>Проект</label>
            <select value={form.projectId} onChange={e => setForm(f => ({
          ...f,
        ...
        {fresh && <div className="adm-link-box">
            <input readOnly value={linkFor(fresh)} onFocus={e => e.target.select()} />
      ...
      <section className="adm-card">
        ...
        {!isEditor ? <p className="adm-hint">Список доступен только в режиме записи.</p> : tokens.length === 0 ? <p className="adm-hint">Пока ни одной.</p> : <table className="adm-table">
            <thead>
              <tr>
                <th>Тип</th>
                <th>Проект</th>
            ...
            return <tr key={t.id}>
                    ...
                    <td>{t.projectName ?? "—"}</td>
                    <td>
                      {t.isUsed ? <span className="adm-hint">использована</span> : expired ? <span className="adm-error">просрочена</span> : <button className="adm-btn adm-btn-small" onClick={() => copy(t.id)}>
                          копировать
                        </button>}
                    </td>
                    <td className="adm-hint">{fmt(t.expiresAt)}</td>
                    <td className="adm-td-right">
                      <button className="adm-del" onClick={() => revoke(t)} aria-label="Отозвать">
                        ✕
                      </button>
                    </td>
                  </tr>;
      ...
      <section className="adm-card">
        ...
        {reviews.map(r => <article className="adm-review" key={r.id}>
            <header>
              <strong>{r.authorName}</strong>
              {(r.position || r.company) && <span className="adm-hint">
              ...
              <button className="adm-del" onClick={() => drop(r)} aria-label="Удалить">
                ✕
              </button>
            </header>
            <p>{r.text}</p>
