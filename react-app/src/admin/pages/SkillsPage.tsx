...
import { ADMIN_SKILLS_QUERY, CREATE_SKILL_MUTATION, DELETE_SKILL_MUTATION, UPDATE_SKILL_MUTATION } from "../api/api";
...
export default function SkillsPage() {
  ...
  const patch = (id: string, changes: Partial<Skill>) => setRows(rs => rs.map(r => r.id === id ? {
    ...r,
    ...changes
  } : r));
  ...
  const commit = async (row: Skill) => {
    ...
    const changed = !before || before.name !== row.name || before.category !== row.category || before.level !== row.level;
    if (!changed) return;
    if (!isEditor) {
      ...
      <table className="adm-table">
        ...
        <tbody>
          {rows.map(row => <tr key={row.id}>
              <td>
                <input value={row.name} onChange={e => patch(row.id, {
              name: e.target.value
            })} onBlur={() => commit({
              ...row
            })} />
              ...
              <td>
                <div className="adm-level" role="group" aria-label={`Уровень ${row.level} из 5`}>
                  {[1, 2, 3, 4, 5].map(n => <button key={n} type="button" aria-label={`Уровень ${n}`} data-on={String(n <= row.level)} onClick={() => {
                const next = {
                  ...row,
                  level: n
                };
          ...
          <tr className="adm-row-new">
            <td>
              <input value={draft.name} placeholder="Новый навык" onChange={e => setDraft(d => ({
              ...d,
              name: e.target.value
            }))} onKeyDown={e => e.key === "Enter" && add()} />
            </td>
            <td>
              <select value={draft.category} onChange={e => setDraft(d => ({
              ...d,
              category: e.target.value
            }))}>
                {CATEGORIES.map(c => <option key={c.value} value={c.value}>
            ...
            <td className="adm-td-right">
              <button className="adm-btn" onClick={add} disabled={!draft.name.trim()}>
                добавить
              </button>
            </td>
          </tr>
