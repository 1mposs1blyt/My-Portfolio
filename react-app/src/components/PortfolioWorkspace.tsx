...
const CATEGORY_ORDER = ["LANGUAGE", "FRONTEND", "BACKEND", "DATABASE", "INFRA", "TOOL"];
export default function PortfolioWorkspace() {
  ...
  const projects = useMemo(() => {
    ...
    return [...profile.projects].sort((a, b) => a.order - b.order).map(project => ({
      ...project,
      reviews: (reviews || []).filter((r: any) => r.type === "CLIENT" && r.projectId === project.id)
    }));
  ...
  const links = useMemo(() => profile?.links ? [...profile.links].sort((a, b) => a.order - b.order) : [], [profile?.links]);
  ...
  const experience = useMemo(() => profile?.experience ? [...profile.experience].sort((a, b) => {
  ...
  const recalc = useCallback(() => {
    ...
    setAvailable(prev => prev.length === ids.length && prev.every((v, i) => v === ids[i]) ? prev : ids);
