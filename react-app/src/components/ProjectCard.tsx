...
export default function ProjectCard({ p, t, onOpen }: ProjectCardProps) {
  ...
  // Проверяем, является ли это сгенерированным кодом или обычной ссылкой
  const isGeneratedSvg = coverUrl.startsWith("data:image/svg+xml,");
  const rawSvgContent = isGeneratedSvg
    ...
    <article className="b-window b-card">
      ...
      {has ? (
        <button
          className="b-cover"
          onClick={() => onOpen(p)}
          aria-label={`${t.gallery} — ${p.name}`}
        >
          {isGeneratedSvg ? (
            /* 💡 Вставляем инлайново чистый SVG без всяких src — CSP бессилен! */
            <div
        ...
        <div className="b-cover b-cover-empty">
          <span>{t.noShots}</span>
        </div>
      )}
      ...
      <div className="b-card-body">
        ...
        <div className="b-actions">
          {p.liveUrl ? (
            ...
            has && (
              <button className="b-btn b-btn-main" onClick={() => onOpen(p)}>
                {t.gallery}
              </button>
            )
          )}
