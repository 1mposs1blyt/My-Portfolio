...
export class ExperienceService {
  ...
  private readonly withTranslations = (lang: Language) => ({
    experienceTranslations: { where: { language: lang } },
    achievements: {
      orderBy: { order: 'asc' as const },
      include: { achievementTranslations: { where: { language: lang } } },
    },
  });
    ...
    const exp = await this.prisma.experience.create({
      data: {
        ...
        experienceTranslations: {
          create: { language: lang, position, description },
        },
        achievements: {
          create: (achievements ?? []).map((text, order) => ({
            order,
            achievementTranslations: { create: { language: lang, text } },
          })),
        },
      },
    });
    return this.findOneWithLang(exp.id, lang); // вернуть уже смаппленный объект
  }
      ...
      data: {
        ...
        ...((position !== undefined || description !== undefined) && {
          experienceTranslations: {
            upsert: {
              where: {
                experienceId_language: { experienceId: id, language: lang },
              },
              create: { language: lang, position: position ?? '', description },
              update: {
